...
export async function processCover(buf: Buffer): Promise<ProcessedCover> {
  ...
  if (aspect < ASPECT_MIN || aspect > ASPECT_MAX) {
    throw new CoverError(
      `Image is ${meta.width}×${meta.height} — covers must be portrait, close to 3:4 (e.g. 1200×1600). Crop it and re-upload.`,
    );
  }
  if (meta.width < MIN_W || meta.height < MIN_H) {
    throw new CoverError(
      `Image is ${meta.width}×${meta.height}px — covers need at least ${MIN_W}×${MIN_H}px so they stay sharp on the app's bookshelf.`,
    );
  }
  const cover = await sharp(buf).resize(MIN_W, MIN_H, { fit: "cover" }).webp({ quality: 82 }).toBuffer();
  const thumb = await sharp(buf).resize(300, 400, { fit: "cover" }).webp({ quality: 80 }).toBuffer();
  return { cover, thumb };
}
