...
Everything decided-but-unbuilt, built-but-unapplied, or deliberately deferred. Updated 2026-08-11.
...
- **A `created` scan's delete stamps `score_scan_detached_at` on a reader who was never shown a pane.** `stampAndDeleteScans` (`notes/scan_delete.ts:31-37`) selects on `read_at IS NOT NULL` and `status IN ('sent','retracted')` and never consults the scan's status, while R8 renders nothing at all for a scan below `ready`. So deleting a scan that never finished uploading gives its recipient R6 — *"The score attached to this note isn't available any more"* — about a score they never saw, which is the same disclosure R7 exists to prevent. Found while splitting D1b off D1; reachable only once a client attaches a `created` scan and sends the note, which `ownedScanId` permits (`notes.ts:194-196` refuses only `taken_down`). The fix is one more predicate in the CTE, and it belongs with whoever owns the attach route, not with a copy change.
