...
| **Nothing ever unregisters a push device — fix before the APNs key lands, not after** | Push, once it works | `registerDevice` is called; **`unregisterDevice` had no caller anywhere** and was deleted as dead code on 2026-08-11. Sign-out does not unregister, and neither does account deletion. Inert today because `apns: null` means no push is attempted — but the day the APNs key is configured, a signed-out device and a deleted account both keep receiving pushes for notes they should no longer see. The route exists server-side; the client call has to be re-added at sign-out and at delete-account, and that is a change worth making in the same session as the key, not discovered after the first push ships. |
